...
║  Versión: 2.3 | Python 3.11+ | Auth: RSA-PSS                   ║
...
def _mejor_ask(libro: dict) -> float:
    """
    Precio mas bajo del libro con cantidad > _LIBRO_CANTIDAD_MINIMA.
    Se exige un volumen minimo real para evitar que niveles residuales
    o mal sincronizados (ej. delta aplicado antes de tiempo, dejando
    una cantidad minuscula en un precio absurdo) se tomen como el
    mejor precio real del mercado.
    """
    precios_validos = [float(p) for p, q in libro.items() if float(q) >= _LIBRO_CANTIDAD_MINIMA]
    return min(precios_validos) if precios_validos else 0.0

def _parse_ob(msg:dict,st:KalshiMarketState):
    """
    Kalshi envia:
      - orderbook_snapshot: libro completo en 'yes_dollars_fp' y 'no_dollars_fp',
        listas de [precio_dolares_str, cantidad_str].
      - orderbook_delta: un cambio puntual {'price_dollars','delta_fp','side'}
        que hay que sumar/restar sobre el libro ya guardado en memoria.
    Se mantiene el libro completo en KalshiMarketState.yes_book/no_book
    para poder aplicar los deltas correctamente y siempre saber el mejor
    precio actual de AMBOS lados (yes_ask y no_ask).
    """
    tipo = msg.get("type")
    cuerpo = msg.get("msg", {})
    ...
    if tipo == "orderbook_snapshot":
        ...
        st.yes_ask = _mejor_ask(st.yes_book)
        st.no_ask  = _mejor_ask(st.no_book)
...
async def _evaluar(shared:SharedState,paper:PaperEngine):
    ...
    if suma_book < 0.85 or suma_book > 1.30:
...
async def main():
    ...
    print("""
╔══════════════════════════════════════════════════════════╗
║   📋  KALSHI BTC BOT — PAPER TRADING  v2.3              ║
