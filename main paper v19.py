...
║  Versión: 1.9 | Python 3.11+ | Auth: RSA-PSS                   ║
...
def _parse_ob(msg:dict,st:KalshiMarketState):
    ...
    # Lado NO — se registra en paralelo, solo para observación (no se opera)
    no_asks=msg.get("msg",{}).get("no",{}).get("asks",[])
    if no_asks and no_asks[0][0] > 0: st.no_ask=no_asks[0][0]/100.0
    elif no_asks and no_asks[0][0] == 0 and len(no_asks) > 1 and no_asks[1][0] > 0: st.no_ask=no_asks[1][0]/100.0
    else: st.no_ask=0.0
...
async def main():
    ...
    print("""
╔══════════════════════════════════════════════════════════╗
║   📋  KALSHI BTC BOT — PAPER TRADING  v1.9              ║
