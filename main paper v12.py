...
║  Versión: 1.2 | Python 3.11+ | Auth: RSA-PSS                   ║
╚══════════════════════════════════════════════════════════════════╝
"""
...
KALSHI_REST        = "https://trading-api.kalshi.com/trade-api/v2"
...
async def _descubrir_mercado(shared:SharedState)->bool:
    ...
    try:
        async with aiohttp.ClientSession() as s:
            async with s.get(
                ...
                params={"series_ticker":KALSHI_SERIES,"status":"active"},
    ...
    log_ex.info(f"Mercado: {ticker} | strike=${strike:,.0f}")
    return True
...
def _parse_ob(msg:dict,st:KalshiMarketState):
    ...
    if asks: st.yes_ask=asks[0][0]/100.0
...
async def run_paper_expiry_poller(shared:SharedState,paper:PaperEngine):
    while not shared.is_shutdown():
        await asyncio.sleep(60)
        async with paper._lock: abiertos=dict(paper.trades_abiertos)
...
async def main():
    ...
    print("""
╔══════════════════════════════════════════════════════════╗
║   📋  KALSHI BTC BOT — PAPER TRADING  v1.2              ║
